...
/*!	\brief Расчет OddRecording
	...
	В [k]oddRecording помещается кодированное представление нечетного слова 
	[n]a (a_0, a_1,..., a_{k-1}) такое, что
	-	a_i \in {\pm 1, \pm 3, ..., \pm 2^{w-1} - 1};
	-	a как число равняется \sum {i=0}^{k-1} a_i 2^{i(w-1)};
	.
	\remark Кодирование символов состоит в следующем
	(\<b> -- двоичная запись числа b):
	-	положительные a_i представляются двоичными символами 0\<a_i> длины w;
	-	отрицательные a_i представляются двоичными символами 1\<|a_i|> длины w;
	...
	k * w битов. Поэтому при w < B_PER_W для хранения всего кодового представления
	потребуется не более k слов.
	\pre 2 <= w < B_PER_W.
	\pre Буфер naf не пересекается с буфером a.
	\pre a - нечетное
	\pre a < 2^{k(w-1)}
