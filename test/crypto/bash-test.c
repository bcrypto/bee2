...
#if defined(BEE2_BASH_AVX2)
bool_t bashavx2_Test0()
{
    octet buf[ 192 ];
    octet hash[ 64 ];
    octet state[ 1024 ];
    // создать стек
    ASSERT( sizeof( state ) >= bashavx2_256_keep() );
    ASSERT( sizeof( state ) >= bashavx2_384_keep() );
    ASSERT( sizeof( state ) >= bashavx2_512_keep() );
    // тест A.1
    memCopy( buf, beltH(), 192 );
    bashavx2_F( buf );
    if( !hexEq( buf,
        ...
        return FALSE;
    // тест A.2.1
    bashavx2_256Hash( hash, beltH(), 0 );
    if( !hexEq( hash,
        "114C3DFAE373D9BCBC3602D6386F2D6A"
        "2059BA1BF9048DBAA5146A6CB775709D" ) )
        return FALSE;
    // тест A.2.2
    bashavx2_256Hash( hash, beltH(), 127 );
    if( !hexEq( hash,
        "3D7F4EFA00E9BA33FEED259986567DCF"
        "5C6D12D51057A968F14F06CC0F905961" ) )
        return FALSE;
    // тест A.2.3
    bashavx2_256Hash( hash, beltH(), 128 );
    if( !hexEq( hash,
        "D7F428311254B8B2D00F7F9EEFBD8F30"
        "25FA87C4BABD1BDDBE87E35B7AC80DD6" ) )
        return FALSE;
    // тест A.2.4
    bashavx2_256Hash( hash, beltH(), 135 );
    if( !hexEq( hash,
        "1393FA1B65172F2D18946AEAE576FA1C"
        "F54FDD354A0CB2974A997DC4865D3100" ) )
        return FALSE;
    // тест A.2.5
    bashavx2_384Hash( hash, beltH(), 95 );
    if( !hexEq( hash,
        "64334AF830D33F63E9ACDFA184E32522"
        "103FFF5C6860110A2CD369EDBC04387C"
        "501D8F92F749AE4DE15A8305C353D64D" ) )
        return FALSE;
    // тест A.2.6
    bashavx2_384Hash( hash, beltH(), 96 );
    if( !hexEq( hash,
        "D06EFBC16FD6C0880CBFC6A4E3D65AB1"
        "01FA82826934190FAABEBFBFFEDE93B2"
        "2B85EA72A7FB3147A133A5A8FEBD8320" ) )
        return FALSE;
    // тест A.2.7
    bashavx2_384Hash( hash, beltH(), 108 );
    if( !hexEq( hash,
        "FF763296571E2377E71A1538070CC0DE"
        "88888606F32EEE6B082788D246686B00"
        "FC05A17405C5517699DA44B7EF5F55AB" ) )
        return FALSE;
    // тест A.2.8
    bashavx2_512Hash( hash, beltH(), 63 );
    if( !hexEq( hash,
        "2A66C87C189C12E255239406123BDEDB"
        "F19955EAF0808B2AD705E249220845E2"
        "0F4786FB6765D0B5C48984B1B16556EF"
        ...
        return FALSE;
    // тест A.2.9
    bashavx2_512Hash( hash, beltH(), 64 );
    if( !hexEq( hash,
        "07ABBF8580E7E5A321E9B940F667AE20"
        "9E2952CEF557978AE743DB086BAB4885"
        "B708233C3F5541DF8AAFC3611482FDE4"
        ...
        return FALSE;
    // тест A.2.10
    bashavx2_512Hash( hash, beltH(), 127 );
    if( !hexEq( hash,
        "526073918F97928E9D15508385F42F03"
        "ADE3211A23900A30131F8A1E3E1EE21C"
        "C09D13CFF6981101235D895746A4643F"
        ...
        return FALSE;
    // тест A.12
    bashavx2_512Hash( hash, beltH(), 192 );
    if( !hexEq( hash,
        "8724C7FF8A2A83F22E38CB9763777B96"
        "A70ABA3444F214C763D93CD6D19FCFDE"
        "6C3D3931857C4FF6CCCD49BD99852FE9"
